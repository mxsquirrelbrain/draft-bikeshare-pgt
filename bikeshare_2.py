...
def get_filters():
    ...
    user_filters = ['month','day','nothing']
    while True:
        user_filter = input('Would you like to filter by month, day or nothing (will display results from full dataset)?').lower()
        if user_filter not in user_filters:
            print('Please select month, day or nothing')
        else:
            print('Thank you for selecting {}'.format(user_filter))
            break
...
def time_stats(df, user_filter):
    ...
    df['month'] = df['Start Time'].dt.strftime('%B') #solution found on https://datascientyst.com/convert-month-number-to-month-name-pandas-dataframe/
